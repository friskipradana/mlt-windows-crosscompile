...
# FIX: resolve MLT_HOME ke ABSOLUTE path lewat Resolve-Path, bukan cuma
# $PSScriptRoot mentah -- supaya kalau script dipanggil dari working
# directory berbeda (mis. run_melt.bat dari lokasi lain), path yang
# dihasilkan tetap benar dan konsisten, bukan relatif ke cwd yang salah.
$env:MLT_HOME = (Resolve-Path -LiteralPath $PSScriptRoot).Path
...
$MeltExe = $null
foreach ($candidate in @("$env:MLT_HOME\bin\melt.exe", "$env:MLT_HOME\melt.exe")) {
    if (Test-Path $candidate) {
        $MeltExe = (Resolve-Path -LiteralPath $candidate).Path
        break
    }
}
...
# FIX (root cause "no plugins found in path yang salah"): PAKSA override
# MLT_REPOSITORY/MLT_DATA/MLT_PROFILES_PATH ke path relatif terhadap
# MLT_HOME yang baru saja di-resolve, TERLEPAS dari apapun yang sudah
# ke-set sebelumnya di System/User Environment Variables Windows atau
# sesi PowerShell lama. Env var permanen yang nyangkut dari percobaan
# sebelumnya adalah penyebab paling umum "no plugins found in <path lama>"
# padahal folder project sudah pindah/benar.
...
# FIX: print diagnostic SEBELUM run, supaya kalau masih salah, kelihatan
# LANGSUNG dari output -- tidak perlu lagi tanya-tanya "env var apa yang
# kepakai". Ini bukan verbose demi verbose; ini satu-satunya cara cepat
# mengonfirmasi path mana yang benar-benar dipakai saat itu juga.
Write-Host "==================== MLT run_melt.ps1 ====================" -ForegroundColor Cyan
Write-Host "MLT_HOME           : $env:MLT_HOME"
Write-Host "melt.exe            : $MeltExe"
...
if (-not (Test-Path $env:MLT_REPOSITORY)) {
    Write-Host "[ERROR] MLT_REPOSITORY tidak ditemukan: $env:MLT_REPOSITORY" -ForegroundColor Red
    Write-Host "  Folder lib\mlt tidak ada di lokasi yang barusan di-resolve di atas."
    Write-Host "  Cek ulang: apakah folder ini benar hasil extract zip yang LENGKAP"
    Write-Host "  (harus ada melt.exe/bin\melt.exe, lib\mlt\*.dll, share\mlt\profiles)?"
    exit 1
}

$ModCount = (Get-ChildItem -LiteralPath $env:MLT_REPOSITORY -Filter "*.dll" -ErrorAction SilentlyContinue | Measure-Object).Count
if ($ModCount -eq 0) {
    Write-Host "[ERROR] MLT_REPOSITORY ($env:MLT_REPOSITORY) ADA tapi KOSONG (0 DLL)!" -ForegroundColor Red
    Write-Host "  Folder-nya ketemu, tapi tidak ada modul .dll di dalamnya."
    exit 1
}
Write-Host "[OK] $ModCount modul DLL ditemukan di MLT_REPOSITORY" -ForegroundColor Green
...
exit $ExitCode
EOF
  echo "[OK] run_melt.ps1 generated (v6, folder polos: mlt, robust ke lokasi melt.exe, forced env override + diagnostic)"
}

# ─── generate run_melt.bat (wrapper supaya tidak kena PowerShell execution
# policy default Windows, user tidak perlu ketik -ExecutionPolicy Bypass
# manual atau ubah setting sistem) ────────────────────────────────────────
generate_run_melt_bat() {
  echo ">>> Generating run_melt.bat..."
  cat > "$PREFIX/run_melt.bat" << 'EOF'
@echo off
REM run_melt.bat (auto-generated by build-all-ubuntu.sh)
REM Wrapper supaya run_melt.ps1 bisa dipanggil tanpa perlu ubah PowerShell
REM execution policy atau ketik -ExecutionPolicy Bypass manual tiap kali.
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run_melt.ps1" %*
EOF
  echo "[OK] run_melt.bat generated"
}
